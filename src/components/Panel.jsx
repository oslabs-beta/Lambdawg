...
const Panel = (props) => {
  ...
  const { setPanelFullScreen, panelFullScreen, msNames, msMetrics, user } = props;
  ...
  if (msNames){
    msNames.forEach((name) => {
      i++;
      names.push(<Microservice name={name} key={`${name}MsEl${i}`} msMetrics={msMetrics} user={user} className='panel-names'/>)
    })
  }
